...
export function DashboardHeader({ year, setYear }: DashboardHeaderProps) {
             ...
             <div className="relative z-50">
                <div className="bg-[#151819] h-[40px] rounded-full flex items-center justify-center px-[4px]">
                  <button 
                    ...
                    className="flex items-center gap-[6px] pl-[16px] pr-[12px] h-full text-[#f2f5f7]"
                  >
                    <span className="font-medium text-[14px]">{year}</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className={cn("transition-transform", isYearOpen && "rotate-180")}>
                ...
                {isYearOpen && (
                  <div className="absolute top-[calc(100%+8px)] right-0 bg-[#151819] border border-[#252a2c] rounded-[12px] p-[4px] min-w-[120px] shadow-xl flex flex-col gap-[2px]">
                    {[2026, 2025, 2024].map((y) => (
                      <button
                        ...
                        className={cn(
                          "px-[12px] py-[8px] rounded-[8px] text-left text-[14px] font-['Titillium_Web',sans-serif]",
                          year === y ? "bg-[#252a2c] text-[#f2f5f7] font-semibold" : "text-[#989ea0] hover:text-[#f2f5f7] hover:bg-[#1e2224]"
                        )}
                      >
                        {y}
                      </button>
