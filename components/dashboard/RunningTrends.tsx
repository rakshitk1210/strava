...
import { aggregateTrendsByDate } from '@/lib/stats-calculator';
...
export function RunningTrends({ runs, unit }: RunningTrendsProps) {
  ...
  const paceData = useMemo(() => aggregateTrendsByDate(runs, 'pace', unit), [runs, unit]);
  const distanceData = useMemo(() => aggregateTrendsByDate(runs, 'distance', unit), [runs, unit]);
  const elevationData = useMemo(() => aggregateTrendsByDate(runs, 'elevation', unit), [runs, unit]);
